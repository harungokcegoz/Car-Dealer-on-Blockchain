...
import { confirmMileageFromContract } from '../../utils/CarContract';
...
const APKConfirmationsPage = () => {
  ...
  const confirmMileageUpdate = async (carId: number) => {
    try {
      ...
      if (!newMileage || Number.isNaN(newMileage)) {
              ...
              {requestedCarsArray.map((car, index) => (
                <div key={index} className="mb-4 flex gap-20 border-b-2 py-7 items-center">
                  ...
                  <div className="flex-1">
                    ...
                    />
                    {mileageErrors[car.carId] && (
                      <p className="text-red-500 text-xs mt-2">
                        New mileage cannot be lower than current mileage. It will be denied by the
                        contract.
                      </p>
                    )}
                  </div>
